...
class State:
	def __init__(self,domainclass=None,problem_file=None,parent_state=None,action=None,action_parameters=None):
		...
		'''
		Since the state is used a the key to the hash table to find out if
		The state has been explored or not it is important to sort the state such
		that it is in the same order or else it would be a different key.
		'''
		self.state = sorted(self.state)

	def set_state_cost(self,solver=None,weight = 0.01):
		if solver:
			solver = solver.lower()
		...
		if solver=='bfs' or solver == "breadth first search":
			...
			self.cost = self.estimated_dist_to_goal + weight * self.depth
		elif solver == 'relaxed':
